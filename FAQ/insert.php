	...
	for ($i=0; $i<$count; $i++)
	{
		...
		if (!$upfile_error[$i])
		{
			...
			if( $upfile_size[$i]  > 1500000 ) {
				echo("
				<script>
				alert('업로드 파일 크기가 지정된 용량(1500KB)을 초과합니다!<br>파일 크기를 체크해주세요! ');
				history.go(-1)
				</script>
				");
				exit;
 ...
 if(  $mode != "modify"  )
	{
		...
		$sql = "insert into $table (id, name, category , subject, content,  regist_day,  ";
		$sql .= " file_name_0, file_name_1, file_name_2, file_copied_0,  file_copied_1, file_copied_2) ";

		$sql .= "values('$id', '$name','$find', '$subject', '$content', '$regist_day', ";
